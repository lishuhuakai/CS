...
elf_t elf_from_multiboot(multiboot_t *mb)
{
	...
	for (i = 0; i < mb->num; i++) {
		...
		if (strcmp(name, ".strtab") == 0) { // 如果是字符表
			elf.strtab = (const char *)sh[i].addr + PAGE_OFFSET; // 字符串表的地址
			elf.strtabsz = sh[i].size; // 字符串表的大小
		}
		if (strcmp(name, ".symtab") == 0) { // 如果是符号表
			elf.symtab = (elf_symbol_t*)sh[i].addr + PAGE_OFFSET;
			elf.symtabsz = sh[i].size;
		}
	} // 按照这种方法，应该.strtab以及.symtab最多仅有一个吧

	return elf; // 好吧，没人能够说清楚到底返回的是符号表还是字符串表
}
