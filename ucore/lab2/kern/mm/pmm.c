...
page_init(void) {
	...
	// 也就是说，pa=0x8000 = (ox8000+KERNBASE)-KERNBASE，所以就是这样喽
...
page_insert(pde_t *pgdir, struct Page *page, uintptr_t la, uint32_t perm) {
	// 我们来看一下是如何实现页面插入的吧！
	// pgdir指的是page directory entry
    pte_t *ptep = get_pte(pgdir, la, 1); // 线性地址la对应的page table entry的地址
    if (ptep == NULL) {
        return -E_NO_MEM; // 到了这里，无非是内存已经耗尽了
    }
    page_ref_inc(page); // 也就是这个页面的引用计数加1
    if (*ptep & PTE_P) { // 这个页面已经存在了的话
        struct Page *p = pte2page(*ptep); // 得到对应的Page
        if (p == page) { // 两个指针其实指向同一个地址
            page_ref_dec(page);
        }
        else { // 如果不是指向同一个地址
            page_remove_pte(pgdir, la, ptep); // 移除对ptep的引用吗？
        }
    }
	// page2pa(page)是将page转换为对应的物理地址
