...
#define N_SLINE     0x44    // text segment line number # 用于表示行号
#define N_DSLINE    0x46    // data segment line number
#define N_BSLINE    0x48    // bss segment line number
#define N_SSYM      0x60    // structure/union element
#define N_SO        0x64    // main source file name # 主源文件的信息
...
// 为什么我会感觉不太对啊，因为这里的结构和书上的结构不太一样
// stab结构一共是12个字节的大小
struct stab {
    uint32_t n_strx;        // index into string table of name # 在字符串表中的索引
    uint8_t n_type;         // type of symbol # symbol的类型
    uint8_t n_other;        // misc info (usually empty) # 一般为0
    uint16_t n_desc;        // description field
    uintptr_t n_value;      // value of symbol # 值
};
