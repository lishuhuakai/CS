...
/*
 * Abstract filesystem. (Or device accessible as a file.) # 抽象文件系统
 *
 * Information:
 *      fs_info   : filesystem-specific data (sfs_fs)		#
 *      fs_type   : filesystem type							# 文件系统的类型
 * Operations:
 *
 *      fs_sync       - Flush all dirty buffers to disk.	# 将修改后的数据写回磁盘
 *      fs_get_root   - Return root inode of filesystem.	# 返回文件系统的根节点
...
struct fs {
    union {
        struct sfs_fs __sfs_info;                  // fs里面居然又包含sfs_fs结构
