...
dev_open(struct inode *node, uint32_t open_flags) {
    ...
    struct device *dev = vop_info(node, device); // 返回node里面devicr的信息
    return dop_open(dev, open_flags); // 调用打开文件的函数
}
...
dev_close(struct inode *node) {
    ...
    return dop_close(dev); // 直接调用关闭文件的函数
}
 ...
 */
// 这里只是tryseek，并没有真正地seek
static int
dev_tryseek(struct inode *node, off_t pos) {
    struct device *dev = vop_info(node, device);
    if (dev->d_blocks > 0) { // 只对块设备进行操作
...
dev_lookup(struct inode *node, char *path, struct inode **node_store) {
    ...
    vop_ref_inc(node); // 对这个node的占用数加1
    *node_store = node; // 事实上，这个函数什么也没有干
    return 0;
}
...
dev_init(void) {
   // init_device(null);
    init_device(stdin); // 标准输入
    init_device(stdout); // 标准输出
    init_device(disk0); // 磁盘
}
/* dev_create_inode - Create inode for a vfs-level device. */
struct inode *
dev_create_inode(void) {
    struct inode *node;
    if ((node = alloc_inode(device)) != NULL) { // 先分配一个inode节点，并且设定该inode的类别是device类型
        vop_init(node, &dev_node_ops, NULL); // 然后将dev_node_ops设置为node->in_ops,记录下对应的操作函数
    }
    return node;
}
