...
static int
disk0_open(struct device *dev, uint32_t open_flags) { // 磁盘都不需要打开的
    return 0;
}

static int
disk0_close(struct device *dev) { // 也不需要关闭
    return 0;
}

static void
disk0_read_blks_nolock(uint32_t blkno, uint32_t nblks) { // 从blkno开始，读取nblks长度的数据
    int ret;
    uint32_t sectno = blkno * DISK0_BLK_NSECT, nsecs = nblks * DISK0_BLK_NSECT;
    if ((ret = ide_read_secs(DISK0_DEV_NO, sectno, disk0_buffer, nsecs)) != 0) { // 都读到buffer里面去了
...
static int
disk0_io(struct device *dev, struct iobuf *iob, bool write) { // 稍微来看一下io吧！
...
static int
disk0_ioctl(struct device *dev, int op, void *data) { // UNIMP表示未曾实现
    return -E_UNIMP;
}
...
disk0_device_init(struct device *dev) {
    ...
    dev->d_blocks = ide_device_size(DISK0_DEV_NO) / DISK0_BLK_NSECT; // 这是一个块设备
    dev->d_blocksize = DISK0_BLKSIZE; // 块的数目
