...
namespace pubsub
{
	...
	class Topic : public muduo::copyable // 居然还有Topic这个类
	{
	public:
	...
	class PubSubServer : boost::noncopyable // 好吧，这个玩意算是主机啦！
	{
	public:
		PubSubServer(muduo::net::EventLoop* loop,
			const muduo::net::InetAddress& listenAddr) 
			: loop_(loop)
			, server_(loop, listenAddr, "PubSubServer") // 构建一个server，是吧！
		{
			server_.setConnectionCallback(
			    boost::bind(&PubSubServer::onConnection, this, _1)); // 设置回调函数
	...
	private:
			...
			Timestamp receiveTime)
		{
			...
			while (result == kSuccess)
			{
				...
				string topic;
				string content;
				result = parseMessage(buf, &cmd, &topic, &content); // 解析数据
				if (result == kSuccess)
				{
					if (cmd == "pub")
					{
