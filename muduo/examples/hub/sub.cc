...
void connection(PubSubClient* client)
{
	if (client->connected())
	{
		for (std::vector<string>::iterator it = g_topics.begin();
		    it != g_topics.end(); ++it)
		{
			client->subscribe(*it, subscription); // 订阅了这些topic
		}
	}
	else
	{
...
int main(int argc, char* argv[])
{
	if (argc > 2)
	{
		string hostport = argv[1]; // 得到端口信息
		size_t colon = hostport.find(':');
		if (colon != string::npos)
		{
			string hostip = hostport.substr(0, colon); // 得到ip信息
			uint16_t port = static_cast<uint16_t>(atoi(hostport.c_str() + colon + 1)); // 得到端口信息
			for (int i = 2; i < argc; ++i)
			{
				g_topics.push_back(argv[i]); // 得到topic信息
			}
